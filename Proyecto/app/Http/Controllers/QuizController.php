...
class QuizController extends Controller
{
  ...
  */
  ...
  public function modalAPAjax(Request $request) {
	...
	{
			//Fantasy Quiz
			$activepoints = ActivePoint::where('id_fantasy', $id_fantasy)->get();
			$quiz = Quiz::where('id_fantasy', $id_fantasy)->get();
			$question_Fantasy = Question::where('id_quiz', $quiz->id )->get();
			$answer_Fantasy = Answer::where('id_question', $question_Fantasy->id)->get();

			//Active Points Quiz
			$quizzes_ActivePoint[] = array();
			foreach( $activepoints as $activepoint){
				array_push($quizzes_ActivePoint, Quiz::where('id_activepoint', $activepoint->id)->get());
			}
